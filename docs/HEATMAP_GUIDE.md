...
│                    ┌─ /api/tile/{L}/{R}/{C} → render on-the-fly         │
│                    │  + disk LRU cache (20 GB, 24h TTL)                │
│                    ├─ /api/obs/range → lazy metadata (chỉ vùng xem)     │
│                    ├─ /api/custom → out-of-core gene subset             │
│                    └─ /tiles/... → static (legacy, small datasets)      │
│                              ↓  HTTP                                    │
│                    [Trình duyệt] Deck.gl                                 │
...
```
Trình duyệt hỏi:  GET /api/tile/2/0/2
                         ↓
...
```
Trình duyệt:  "Cho tôi ảnh /api/tile/2/0/2"
                     ↓
Máy chủ:      1. Kiểm tra disk cache → có? → trả ngay
...
```
GET  /api/meta                 → Siêu dữ liệu (kích thước, cấp, vmin/vmax, cụm)
GET  /api/tile/{level}/{row}/{col}  → Dynamic tile (render + cache)  ← MẶC ĐỊNH
GET  /tiles/{level}/{row}_{col}.png → Static PNG (legacy, dataset nhỏ)
GET  /api/obs                  → Toàn bộ metadata tế bào (chỉ < 1M cells)
GET  /api/obs/range?start=&end= → Metadata cho range [start, end)  ← LAZY
GET  /api/var                  → Tên gen
GET  /api/groups               → Danh sách cụm (id + kích thước)
GET  /api/cell_order           → Mảng hoán vị tế bào (từ Zarr)
GET  /api/cache/stats          → Thống kê disk cache (hit/miss/size)
POST /api/custom               → Tạo kim tự tháp tùy chỉnh (chọn gen)
GET  /api/custom/{id}/tile     → Ảnh ô của kim tự tháp tùy chỉnh
GET  /api/custom/{id}/meta     → Metadata kim tự tháp tùy chỉnh
```

> **Thay đổi v0.2:**
> - `/api/tile/{L}/{R}/{C}` là endpoint **mặc định** (dynamic + cache), thay thế `/tiles/...png` tĩnh
...
│  → GET /api/obs/range?start=18000&end=20000 │
...
│       ↓                                  │
│  5. Mỗi layer tải /api/tile (dynamic)   │
...
tile_render.py: render_tile_png(values, vmin, vmax)
  ...
  │
  ▼
HTTP: GET /api/tile/{L}/{R}/{C} → PNG 256×256 grayscale
...
│  ║       │                                                         ║        │
│  ║       ▼                                                         ║        │
│  ║  data/heatmap.zarr/ (nén zstd, ~150-250 GB)                    ║        │
...
│  ║   ├── GET /api/tile/{L}/{R}/{C} → dynamic + disk cache (LRU)  ║        │
│  ║   │   └── asyncio.to_thread → tile_cache.get_or_render()      ║        │
│  ║   ├── GET /api/obs/range?start=&end= → lazy metadata          ║        │
│  ║   ├── GET /api/obs → 413 nếu ≥ 1M cells                       ║        │
│  ║   ├── GET /api/cell_order, /api/cache/stats                   ║        │
...
│  ║   7. createTileLayers() → GroupedHeatmapLayer mỗi ô           ║      │
│  ║   8. createGapOverlayLayers() → che khe                       ║      │
│  ║   9. createTileBorderLayer() → viền + hover                    ║      │
│  ║  10. createClusterAnnotationLayers() → tên cụm               ║      │
│  ║  11. createAxisLayers() → nhãn trục                            ║      │
│  ║       │                                                         ║      │
│  ║       ▼                                                         ║      │
│  ║  deck.gl (WebGL2)                                               ║      │
...
> **Kết luận:** Kiến trúc v0.2 biến một bảng số **400 tỷ ô** (20M tế bào × 20K gen) thành heatmap tương tác 60 FPS, bằng cách: out-of-core streaming giữ RAM < 8 GB, dynamic tile + disk cache thay 6,2M file tĩnh, RTC giải Float32 jitter, index-based culling thay quét toàn bộ, và lazy metadata fetch tránh browser OOM.
...
build_pyramid.py → data/heatmap.zarr
                      ...
                      │     ├── /api/tile/{L}/{R}/{C} → dynamic + disk cache
                      │     ├── /api/obs/range → lazy metadata
                      │     ├── /api/value (hover tooltip)
                      │     └── /api/custom → out-of-core gene subset pyramid
                      └── debug / phân tích
```
